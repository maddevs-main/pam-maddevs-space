...
const PlusButton = styled.button`
  ...
  box-shadow: 0 6px 20px rgba(var(--color-dark-rgb),0.18);
...
export default function ConsumerProjectPage({ params }: { params: { id: string } }) {
  const { id } = params;
...
function ProjectDetails({ projectId }: { projectId: string }) {
  ...
  // clamp and derive styles for the mean progress bar
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
                      {stages.map((s:any) => {
                        const active = (s.progress || 0) > 0 && (s.progress || 0) < 100;
                          // Map progress -> panel color per spec:
                          // 0 -> upcoming (yellow), 1-99 -> ongoing (green), 100 -> completed (grey/dark)
                          ...
                          // Choose readable text color for right column based on the chosen panelColor
