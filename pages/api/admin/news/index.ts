...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      const items = await db.collection('news').find(q).sort({ date: -1 }).toArray();
      return res.json({ ok: true, news: items.map(n => ({ ...n, id: n._id.toString() })) });
    } catch (err: any) {
      console.error(err);
  ...
  if (req.method === 'POST') {
    ...
    try {
      const r = await db.collection('news').insertOne(doc as any);
